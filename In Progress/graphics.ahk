...
global iSearch_imageSpacingTolerance := 10

; Finds coordinates of all instances of a given image.
FindImageCoordsWithinArea(imagePath, X, Y, W, H, recurseDepth = 1) {
	...
	if(ErrorLevel = 0) { ; We found something, store and recurse!
		...
		; MouseMove, %X%, %Y%
		; DEBUG.popup(X, "X", Y, "Y", recurseDepth, "Recurse Depth")
		; ; Sleep, 500
		; CoordMode, Mouse, Screen
		
		outStr = %outX%-%outY%
		recurseDepth++
		firstXY := FindImageCoordsWithinArea(imagePath, outX + iSearch_imageSpacingTolerance, outY, W, H, recurseDepth)
		recurseDepth++
		secondXY := FindImageCoordsWithinArea(imagePath, X, outY + iSearch_imageSpacingTolerance, outX, H, recurseDepth)
...
ClickWhereFindImage(imagePath, nnClass = "") {
	...
	Loop, %coordsCount% {
		...
		; DEBUG.popup(controlNN, "ControlNN", splitCoords%A_Index%_1, "X", splitCoords%A_Index%_2, "Y")
